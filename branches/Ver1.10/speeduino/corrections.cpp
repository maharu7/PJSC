...
void initialiseCorrections(void)
{
  ...
  currentStatus.tpsDOTdfcoActive = false;     //[PJSC v1.10]
}
...
uint16_t correctionsFuel(void)
{
  ...
  if (configPage15.swWUE == 0) { currentStatus.wueCorrection = 100; }                                                                          //[PJSC v1.03]
  ...
  //******************************** [PJSC v1.10] ********************************
  if (configPage15.swBatVCorrection == 0) { currentStatus.batCorrection = 100; }
  inj_opentime_uS = configPage2.injOpen * currentStatus.batCorrection / 10;
  if( configPage2.indInjAng )
  {
    inj2_opentime_uS = configPage15.injOpen2 * currentStatus.batCorrection / 10;
    inj3_opentime_uS = configPage15.injOpen3 * currentStatus.batCorrection / 10;
    inj4_opentime_uS = configPage15.injOpen4 * currentStatus.batCorrection / 10;
  }
  else
  {
    inj2_opentime_uS = inj_opentime_uS;
    inj3_opentime_uS = inj_opentime_uS;
    inj4_opentime_uS = inj_opentime_uS;
  }
  //******************************** [PJSC v1.10] ********************************
  if (configPage2.battVCorMode == BATTV_COR_MODE_OPENTIME)
  {
//[PJSC v1.10]    inj_opentime_uS = configPage2.injOpen * currentStatus.batCorrection; // Apply voltage correction to injector open time.
    currentStatus.batCorrection = 100; //[PJSC v1.10] This is to ensure that the correction is not applied twice. There is no battery correction fator as we have instead changed the open time
  }
  ...
  if (configPage15.swIATcorrection == 0) { currentStatus.iatCorrection = 100; }                                                                //[PJSC v1.03]
  ...
  if (configPage2.baroCorr == 0) { currentStatus.baroCorrection = 100; }                                                                       //[PJSC v1.10]
...
//[PJSC v1.10]      else if( (configPage2.aeMode == AE_MODE_TPS) && (abs(currentStatus.tpsDOT) > activateTPSDOT) )
      else if( (configPage2.aeMode == AE_MODE_TPS) && (abs(currentStatus.tpsDOT) > activateTPSDOT) && (currentStatus.tpsDOTdfcoActive == false) )      //[PJSC v1.10]
      {
        BIT_CLEAR(currentStatus.engine, BIT_ENGINE_ACC);
        BIT_CLEAR(currentStatus.engine, BIT_ENGINE_DCC);
      }
...
//[PJSC v1.10]        if (abs(currentStatus.tpsDOT) > configPage2.taeThresh)
        if ( (currentStatus.tpsDOT > configPage2.taeThresh) && (currentStatus.tpsDOTdfcoActive == false) )           //[PJSC v1.10]
        {
        ...
        //**************************** [PJSC v1.10] For TPSdotDFCO ****************************
        else if  (currentStatus.tpsDOT <= configPage15.dfcoTPSdotThresh)
        {
          if( !BIT_CHECK(currentStatus.engine, BIT_ENGINE_ACC) )
          {
            if ( (configPage15.dfcoTPSdotEnabled == 1) && (currentStatus.RPM > ( configPage15.dfcoTPSdotRPM * 50)) && (currentStatus.TPS < configPage15.dfcoTPSdotTPSThresh) ) { accelValue = setDecelerationCorrection(); }
            else { accelValue = 100; }
          }
        }
        //**************************** [PJSC v1.10] For TPSdotDFCO ****************************
...
/*
[PJSC v1.10] Set deceleration for SMART shift
*/
int16_t setDecelerationCorrection(void)
{
  int16_t decelValue = 100;

  if( !BIT_CHECK(currentStatus.engine, BIT_ENGINE_DCC) ) { currentStatus.AEEndTime = micros_safe() + ((unsigned long)configPage15.dfcoTPSdotDuration * 10000); }  //Set the time in the future where the fuel cut off will be turned off. dfcoTPSdotDuration is stored as mS / 10, so multiply it by 100 to get it in uS
  BIT_SET(currentStatus.engine, BIT_ENGINE_DCC); //Mark deaccleration fuel cut off as active.
  currentStatus.tpsDOTdfcoActive = true;
  decelValue = configPage15.dfcoTPSdotMulti;
  currentStatus.AEamount = decelValue;
...
bool correctionDFCO(void)
{
  ...
  if ( configPage2.dfcoEnabled == 1 )
  {
    ...
    {
//[PJSC v1.03]      DFCOValue = ( currentStatus.RPM > ( configPage4.dfcoRPM * 10) ) && ( currentStatus.TPS < configPage4.dfcoTPSThresh ); 
      DFCOValue = ( currentStatus.RPM > ( configPage4.dfcoRPM * 50) ) && ( currentStatus.TPS < configPage4.dfcoTPSThresh );                //[PJSC v1.03]
      if ( DFCOValue == false) { dfcoDelay = 0; }
    }
    else 
    {
//[PJSC v1.03]      if ( (currentStatus.TPS < configPage4.dfcoTPSThresh) && (currentStatus.coolant >= (int)(configPage2.dfcoMinCLT - CALIBRATION_TEMPERATURE_OFFSET)) && ( currentStatus.RPM > (unsigned int)( (configPage4.dfcoRPM * 10) + (configPage4.dfcoHyster * 2)) ) )
      if ( (currentStatus.TPS < configPage4.dfcoTPSThresh) && (currentStatus.coolant >= (int)(configPage2.dfcoMinCLT - CALIBRATION_TEMPERATURE_OFFSET)) && ( currentStatus.RPM > (unsigned int)( (configPage4.dfcoRPM * 50) + configPage4.dfcoHyster) ) )       //[PJSC v1.03]
      {
