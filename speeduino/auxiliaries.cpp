...
//[PJSC v1.10]static long vvt2_pwm_value;
volatile unsigned int vvt1_pwm_cur_value;
//[PJSC v1.10]volatile unsigned int vvt2_pwm_cur_value;
static long vvt_pid_target_angle;
//[PJSC v1.10]static long vvt2_pid_target_angle;
static long vvt_pid_current_angle;
//[PJSC v1.10]static long vvt2_pid_current_angle;
volatile bool vvt1_pwm_state;
//[PJSC v1.10]volatile bool vvt2_pwm_state;
volatile bool vvt1_max_pwm;
//[PJSC v1.10]volatile bool vvt2_max_pwm;
...
//[PJSC v1.10]volatile PORT_TYPE *vvt2_pin_port;
//[PJSC v1.10]volatile PINMASK_TYPE vvt2_pin_mask;
volatile PORT_TYPE *fan_pin_port;
volatile PINMASK_TYPE fan_pin_mask;
volatile PORT_TYPE *mux1_pin_port;            //[PJSC v1.10]
...
volatile PORT_TYPE *mux4_pin_port;            // |
volatile byte mux4_pin_mask;                  // |
volatile PORT_TYPE *muxHC_pin_port;           // |
volatile byte muxHC_pin_mask;                 // |
volatile PORT_TYPE *pvdir_pin_port;           // |
volatile PINMASK_TYPE pvdir_pin_mask;         // |
volatile PORT_TYPE *pvpwm_pin_port;           // |
volatile PINMASK_TYPE pvpwm_pin_mask;         // |
volatile PORT_TYPE *pvdis_pin_port;           // |
volatile PINMASK_TYPE pvdis_pin_mask;         // |
volatile PORT_TYPE *starter_pin_port;         // V
volatile PINMASK_TYPE starter_pin_mask;       //[PJSC v1.10]
...
//[PJSC v1.10]integerPID vvt2PID(&vvt2_pid_current_angle, &currentStatus.vvt2Duty, &vvt2_pid_target_angle, configPage10.vvtCLKP, configPage10.vvtCLKI, configPage10.vvtCLKD, configPage4.vvt2PWMdir); //This is the PID object if that algorithm is used. Needs to be global as it maintains state outside of each function call
...
void fanControl(void)
{
  if( configPage2.fanEnable == 1 ) // regular on/off fan control
  {
    ...
    [PJSC v1.10] Omitto Ari conditoner controll */
    if ( (fanPermit == true) && (currentStatus.coolant >= onTemp) )   //[PJSC v1.10] Omitto Ari conditoner controll
    {
...
//[PJSC v1.10]  vvt2_pin_port = portOutputRegister(digitalPinToPort(pinVVT_2));
//[PJSC v1.10]  vvt2_pin_mask = digitalPinToBitMask(pinVVT_2);
  ...
  mux3_pin_mask = digitalPinToBitMask(pinMuxout3);                     // |
  mux4_pin_port = portOutputRegister(digitalPinToPort(pinMuxout4));    // V
  mux4_pin_mask = digitalPinToBitMask(pinMuxout4);                     //[PJSC v1.01]
  muxHC_pin_port = portOutputRegister(digitalPinToPort(pinMuxoutHC));  //[PJSC v1.10]
  muxHC_pin_mask = digitalPinToBitMask(pinMuxoutHC);                   //[PJSC v1.10]
...
//[PJSC v1.10]    currentStatus.vvt2Angle = 0;

    #if defined(CORE_AVR)
//[PJSC v1.10]      vvt_pwm_max_count = (uint16_t)(MICROS_PER_SEC / (16U * configPage6.vvtFreq * 2U)); //Converts the frequency in Hz to the number of ticks (at 16uS) it takes to complete 1 cycle. Note that the frequency is divided by 2 coming from TS to allow for up to 512hz
      vvt_pwm_max_count = (uint16_t)(MICROS_PER_SEC / (16U * configPage6.vvtFreq)); //[PJSC v1.10]
    #elif defined(CORE_TEENSY35)
//[PJSC v1.10]      vvt_pwm_max_count = (uint16_t)(MICROS_PER_SEC / (32U * configPage6.vvtFreq * 2U)); //Converts the frequency in Hz to the number of ticks (at 16uS) it takes to complete 1 cycle. Note that the frequency is divided by 2 coming from TS to allow for up to 512hz
      vvt_pwm_max_count = (uint16_t)(MICROS_PER_SEC / (32U * configPage6.vvtFreq)); //[PJSC v1.10]
    #elif defined(CORE_TEENSY41)
//[PJSC v1.10]      vvt_pwm_max_count = (uint16_t)(MICROS_PER_SEC / (2U * configPage6.vvtFreq * 2U)); //Converts the frequency in Hz to the number of ticks (at 2uS) it takes to complete 1 cycle. Note that the frequency is divided by 2 coming from TS to allow for up to 512hz
      vvt_pwm_max_count = (uint16_t)(MICROS_PER_SEC / (2U * configPage6.vvtFreq)); //[PJSC v1.10]
    #endif
...
//[PJSC v1.10]  if( (configPage6.vvtEnabled == 0) && (configPage10.wmiEnabled >= 1) )
  if( (configPage6.vvtEnabled == 0) && ((configPage10.wmiEnabled >= 1) || (configPage15.oilSolenoidEnabled == 1)) )   //[PJSC v1.10]
  {
...
void vvtControl(void)
{
  if( (configPage6.vvtEnabled == 1) && (currentStatus.coolant >= (int)(configPage4.vvtMinClt - CALIBRATION_TEMPERATURE_OFFSET)) && (BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN)))
  {
    ...
    //[PJSC v1.10]if( configPage4.TrigPattern == 9 ) { currentStatus.vvt1Angle = getCamAngle_Miata9905(); }
    ...
    if( (vvtIsHot == true) || ((runSecsX10 - vvtWarmTime) >= (configPage4.vvtDelay * VVT_TIME_DELAY_MULTIPLIER)) ) 
    {
      ...
      if( configPage10.wmiEnabled == 0 ) //Added possibility to use vvt and wmi at the same time
      {
        if( (currentStatus.vvt1Duty == 0) && (currentStatus.vvt2Duty == 0) )
        {
          ...
          VVT1_PIN_OFF();
//[PJSC v1.10]          VVT2_PIN_OFF();
          vvt1_pwm_state = false;
          vvt1_max_pwm = false;
//[PJSC v1.10]          vvt2_pwm_state = false;
//[PJSC v1.10]          vvt2_max_pwm = false;
          DISABLE_VVT_TIMER();
        }
        else if( (currentStatus.vvt1Duty >= 200) && (currentStatus.vvt2Duty >= 200) )
        {
          ...
          VVT1_PIN_ON();
//[PJSC v1.10]          VVT2_PIN_ON();
          vvt1_pwm_state = true;
          vvt1_max_pwm = true;
//[PJSC v1.10]          vvt2_pwm_state = true;
//[PJSC v1.10]          vvt2_max_pwm = true;
          DISABLE_VVT_TIMER();
        }
        else
        {
...
//[PJSC v1.10]          if(currentStatus.vvt2Duty < 200) { vvt2_max_pwm = false; }
        }
      }
      else
      {
  ...
  { 
    if (configPage10.wmiEnabled == 0)
    {
      ...
      DISABLE_VVT_TIMER();
//[PJSC v1.10]      currentStatus.vvt2Duty = 0;
//[PJSC v1.10]      vvt2_pwm_value = 0;
//[PJSC v1.10]      vvt2_pwm_state = false;
//[PJSC v1.10]      vvt2_max_pwm = false;
    }
    if (configPage15.oilSolenoidEnabled == 0)      //[PJSC v1.10]
    {                                              //[PJSC v1.10]
      DISABLE_VVT_TIMER();                         //[PJSC v1.10]
      currentStatus.vvt1Duty = 0;
      vvt1_pwm_value = 0;
      vvt1_pwm_state = false;
      vvt1_max_pwm = false;
...
void wmiControl(void)
{
  ...
  //[PJSC v1.10]if( (configPage10.vvt2Enabled == 0) && (configPage10.wmiEnabled >= 1) )
  if( configPage10.wmiEnabled >= 1 )   //[PJSC v1.10]
  {
...
//[PJSC v1.10]    vvt2_pwm_value = halfPercentage(currentStatus.wmiPW, vvt_pwm_max_count);
...
#endif
{
  if ( BIT_CHECK(currentStatus.muxTestModeActive, BIT_TEST_MUX1) ) { mux1Toggle(); }                                                         //[PJSC v1.10]
  else{                                                                                                                                      //[PJSC v1.10]
...
#endif
{
  if ( BIT_CHECK(currentStatus.muxTestModeActive, BIT_TEST_MUX2) ) { mux2Toggle(); }                                                         //[PJSC v1.10]
  else{                                                                                                                                      //[PJSC v1.10]
//[PJSC v1.10]    if ( ((vvt1_pwm_state == false) || (vvt1_max_pwm == true)) && ((vvt2_pwm_state == false) || (vvt2_max_pwm == true)) )
    if ( (vvt1_pwm_state == false) || (vvt1_max_pwm == true) )                                                                               //[PJSC v1.10]
    {
...
//[PJSC v1.10]      if( (vvt1_pwm_state == true) && ((vvt1_pwm_value <= vvt2_pwm_value) || (vvt2_pwm_state == false)) )
      if( vvt1_pwm_state == true )     //[PJSC v1.10]
      {
        SET_COMPARE(VVT_TIMER_COMPARE, VVT_TIMER_COUNTER + vvt1_pwm_value);
        vvt1_pwm_cur_value = vvt1_pwm_value;
//[PJSC v1.10]        vvt2_pwm_cur_value = vvt2_pwm_value;
//[PJSC v1.10]        if (vvt1_pwm_value == vvt2_pwm_value) { nextVVT = 2; } //Next event is for both PWM
//[PJSC v1.10]        else { nextVVT = 0; } //Next event is for PWM0
        nextVVT = 0;                   //[PJSC v1.10]
      }
      /* [PJSC v1.10]
    ...
    else
    {
      if(nextVVT == 0)
      {
        ...
        nextVVT = 1; //Next event is for PWM1
//[PJSC v1.10]        if(vvt2_pwm_state == true){ SET_COMPARE(VVT_TIMER_COMPARE, VVT_TIMER_COUNTER + (vvt2_pwm_cur_value - vvt1_pwm_cur_value) ); }
//[PJSC v1.10]        else
//[PJSC v1.10]        { 
...
void mux1Toggle(void)
{
  ...
  {
    openMux1();
    SET_COMPARE(BOOST_TIMER_COMPARE, BOOST_TIMER_COUNTER + pjsc_pwm_target_value[CH_MUX1] );
    pjsc_pwm_cur_value[CH_MUX1] = pjsc_pwm_target_value[CH_MUX1];
    pjsc_pwm_state[CH_MUX1] = true;
  }
}

//************************** [PJSC v1.10] Toggle MUX2 output **************************
void mux2Toggle(void)
{
  ...
  {
    openMux2();
    SET_COMPARE(VVT_TIMER_COMPARE, VVT_TIMER_COUNTER + pjsc_pwm_target_value[CH_MUX2] );
    pjsc_pwm_cur_value[CH_MUX2] = pjsc_pwm_target_value[CH_MUX2];
    pjsc_pwm_state[CH_MUX2] = true;
  }
}

//******************** [PJSC v1.10] PV control function ********************
void initialisePvControl(void)
{
  pvdir_pin_port = portOutputRegister(digitalPinToPort(pinPvDIR));
  pvdir_pin_mask = digitalPinToBitMask(pinPvDIR);
  pvpwm_pin_port = portOutputRegister(digitalPinToPort(pinPvPWM));
  pvpwm_pin_mask = digitalPinToBitMask(pinPvPWM);
  pvdis_pin_port = portOutputRegister(digitalPinToPort(pinPvDIS));
  pvdis_pin_mask = digitalPinToBitMask(pinPvDIS);

  pinMode(pinPvDIR, OUTPUT);
...
{
  long tempPVPositionADC;
  tempPVPositionADC = (configPage15.PVPosMax - configPage15.PVPosMin) * currentStatus.PVTargetPosition;
...
void PvControl(void)
{
  if( configPage15.PVControlType > 0 )
  {
    ...
    {
      if( configPage15.PVPowerupTestEnabled && !BIT_CHECK(pv_flag, BIT_PV_TEST_COMP) ) { PvTest(); }
    ...
    /* Stuck check */
/*    if( ((pv_operation == PV_OPE_FORWARD) || (pv_operation == PV_OPE_BACKWARD)) && (pv_target_position_adc != pv_pid_current_position_adc) )
    {
      if( pv_pid_current_position_adc == pv_pid_last_position_adc )
      {
        pv_stuck_check_count++;
        if( (pv_stuck_check_count > (PV_STUCK_THRESHOLD * 10)) && (pv_state != PV_STATE_STUCK) )
        {
          pv_operation_prev = pv_operation;
          pv_state_prev = pv_state;
          pv_state = PV_STATE_STUCK;
          pv_operation = PV_OPE_STOP;
        }
      }
      else if( (pv_state == PV_STATE_STUCK) && (pv_operation != pv_operation_prev) )
      {
        pv_stuck_check_count = 0;
        pv_state = pv_state_prev;
      }
      else
      {
        pv_stuck_check_count = 0;
        if( pv_state == PV_STATE_STUCK ) { pv_state = pv_state_prev; }
      }
    }
*/
    /* PV operate */
...
{
  if( (configPage15.oilSolenoidEnabled == 1) && (configPage6.vvtEnabled == 0) && (BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN)))
  {
    //Lookup VVT duty based on either MAP or TPS
    if(configPage6.vvtLoadSource == VVT_LOAD_TPS) { currentStatus.vvt1Duty = get3DTableValue(&vvtTable, (currentStatus.TPS * 2), currentStatus.RPM); }
    else { currentStatus.vvt1Duty = get3DTableValue(&vvtTable, (currentStatus.MAP), currentStatus.RPM); }

    //if( currentStatus.vvt1Duty < 200 ) { currentStatus.vvt1Duty = 0; }
...
//******************** [PJSC v1.10] Starter control function ********************
/*
Starter control
*/
void initialiseStarter(void)
{
  starter_pin_port = portOutputRegister(digitalPinToPort(pinStarter));
  starter_pin_mask = digitalPinToBitMask(pinStarter);
  STARTER_DISABLE();
}
