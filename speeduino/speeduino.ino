...
uint16_t inj2_opentime_uS = 0;       //[PJSC v1.10]
uint16_t inj3_opentime_uS = 0;       //[PJSC v1.10]
uint16_t inj4_opentime_uS = 0;       //[PJSC v1.10]
uint16_t primaryPW;                  //[PJSC v1.10]
uint16_t secondaryPW;                //[PJSC v1.10]
uint16_t primaryPW_2;                //[PJSC v1.10]
uint16_t secondaryPW_2;              //[PJSC v1.10]
...
void __attribute__((always_inline)) loop(void)
{
    ...
    else
    {
      ...
      fuelChannelsOn = 0;
//[PJSC v1.10]      if (currentStatus.fpPrimed == true) { FUEL_PUMP_OFF(); currentStatus.fuelPumpOn = false; } //Turn off the fuel pump, but only if the priming is complete
      if ( (currentStatus.fpPrimed == true) && !(BIT_CHECK(currentStatus.testModeActive, BIT_TEST_FP)) ) { FUEL_PUMP_OFF(); currentStatus.fuelPumpOn = false; }   //[PJSC v1.10]
    ...
    if(BIT_CHECK(LOOP_TIMER, BIT_TIMER_30HZ)) //30 hertz
    {
      ...
      if(currentStatus.testOutputs == 0)          //[PJSC v1.10] For MUX output test mode
      {                                           //[PJSC v1.10] For MUX output test mode
        ...
        wmiControl();
        //Oil Solenoid
        oilSolenoidControl();                     //[PJSC v1.10] For Oil Solenoid control
      }                                           //[PJSC v1.10] For MUX output test mode
      ...
      if (configPage2.canWBO == 0)
      {
        ...
        //*************** [PJSC v1.10] AFR sensor selection for tune analyze VE ********************
        //if(configPage15.afr_sensor_selection1) { currentStatus.afr_analyze1 = currentStatus.O2_2; }
        //else { currentStatus.afr_analyze1 = currentStatus.O2; }
        //
        //if(configPage15.afr_sensor_selection2) { currentStatus.afr_analyze2 = currentStatus.O2_2; }
        //else { currentStatus.afr_analyze2 = currentStatus.O2; }
        //
        //if(configPage15.afr_sensor_selection3) { currentStatus.afr_analyze3 = currentStatus.O2_2; }
        //else { currentStatus.afr_analyze3 = currentStatus.O2; }
        //
        //if(configPage15.afr_sensor_selection4) { currentStatus.afr_analyze4 = currentStatus.O2_2; }
    ...
    if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_15HZ)) //Every 32 loops
    {
      ...
      if( (configPage2.pinMapping == 70) && (configPage15.analogInputPortSelection == ANALOG_EXVALVE) ) { readPVPosition(); }  //[PJSC v1.02] For External Trigger
...
//[PJSC v1.10]      readBaro(); //Infrequent baro readings are not an issue.
    ...
    selectVEtable();             //[PJSC v1.10]
//[PJSC v1.10]    calculateSecondaryFuel();
...
//[PJSC v1.10]      currentStatus.PW1 = PW(req_fuel_uS, currentStatus.VE, currentStatus.MAP, currentStatus.corrections, inj_opentime_uS);
      ...
      { 
        ...
        currentStatus.PW2 = currentStatus.PW2 + (configPage10.n2o_stage1_adderMax + percentage(adderPercent, (configPage10.n2o_stage1_adderMin - configPage10.n2o_stage1_adderMax))) * 100; //[PJSC v1.10]
        currentStatus.PW3 = currentStatus.PW3 + (configPage10.n2o_stage1_adderMax + percentage(adderPercent, (configPage10.n2o_stage1_adderMin - configPage10.n2o_stage1_adderMax))) * 100; //[PJSC v1.10]
        currentStatus.PW4 = currentStatus.PW4 + (configPage10.n2o_stage1_adderMax + percentage(adderPercent, (configPage10.n2o_stage1_adderMin - configPage10.n2o_stage1_adderMax))) * 100; //[PJSC v1.10]
      }
      if( (currentStatus.nitrous_status == NITROUS_STAGE2) || (currentStatus.nitrous_status == NITROUS_BOTH) )
      {
        ...
        currentStatus.PW2 = currentStatus.PW2 + (configPage10.n2o_stage2_adderMax + percentage(adderPercent, (configPage10.n2o_stage2_adderMin - configPage10.n2o_stage2_adderMax))) * 100; //[PJSC v1.10]
        currentStatus.PW3 = currentStatus.PW3 + (configPage10.n2o_stage2_adderMax + percentage(adderPercent, (configPage10.n2o_stage2_adderMin - configPage10.n2o_stage2_adderMax))) * 100; //[PJSC v1.10]
        currentStatus.PW4 = currentStatus.PW4 + (configPage10.n2o_stage2_adderMax + percentage(adderPercent, (configPage10.n2o_stage2_adderMin - configPage10.n2o_stage2_adderMax))) * 100; //[PJSC v1.10]
      }
...
#if INJ_CHANNELS >= 1
      if( (configPage15.squirtDeviceTypeCh1 == 1) && (configPage2.injLayout != INJ_SEMISEQUENTIAL) ) { pjscControl(CH_INJ1); }   //[PJSC v1.10]
      else {                                                                                                                     //[PJSC v1.03]
        ...
        */
#if INJ_CHANNELS >= 2
      if( (configPage15.squirtDeviceTypeCh2 == 1) && (configPage2.injLayout != INJ_SEMISEQUENTIAL) ) { pjscControl(CH_INJ2); }   //[PJSC v1.10]
      else {                                                                                                                     //[PJSC v1.10]
//[PJSC v1.10]        if( (maxInjOutputs >= 2) && (currentStatus.PW2 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ2_CMD_BIT)) )
        if( (maxInjOutputs >= 2) && (currentStatus.PW2 >= inj2_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ2_CMD_BIT)) )       //[PJSC v1.10]
        {
...
#if INJ_CHANNELS >= 3
      if( (configPage15.squirtDeviceTypeCh3 == 1) && (configPage2.injLayout != INJ_SEMISEQUENTIAL) ) { pjscControl(CH_INJ3); }   //[PJSC v1.10]
      else {                                                                                                                     //[PJSC v1.10]
//[PJSC v1.10]        if( (maxInjOutputs >= 3) && (currentStatus.PW3 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ3_CMD_BIT)) )
        if( (maxInjOutputs >= 3) && (currentStatus.PW3 >= inj3_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ3_CMD_BIT)) )       //[PJSC v1.10]
        {
...
#if INJ_CHANNELS >= 4
      if( (configPage15.squirtDeviceTypeCh4 == 1) && (configPage2.injLayout != INJ_SEMISEQUENTIAL) ) { pjscControl(CH_INJ4); }   //[PJSC v1.10]
      else {                                                                                                                     //[PJSC v1.10]
//[PJSC v1.10]        if( (maxInjOutputs >= 4) && (currentStatus.PW4 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ4_CMD_BIT)) )
        if( (maxInjOutputs >= 4) && (currentStatus.PW4 >= inj4_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ4_CMD_BIT)) )       //[PJSC v1.10]
        {
...
uint16_t PW(int REQ_FUEL, byte VE, long MAP, uint16_t corrections, int injOpen)
{
  ...
  uint16_t iVE;
//[PJSC v1.10]  uint16_t iMAP = 100;
  long iMAP = 1000;              //[PJSC v1.10]
  uint16_t iAFR = 147;
...
//[PJSC v1.10]  if ( configPage2.multiplyMAP == MULTIPLY_MAP_MODE_100) { iMAP = div100( ((uint16_t)MAP << 7U) ); }
//[PJSC v1.10]  else if( configPage2.multiplyMAP == MULTIPLY_MAP_MODE_BARO) { iMAP = ((unsigned int)MAP << 7U) / currentStatus.baro; }
  if ( configPage2.multiplyMAP == MULTIPLY_MAP_MODE_100) { iMAP = (MAP << 7U) / 1000; }                          //[PJSC v1.10]
...
//[PJSC v1.10]    if ( BIT_CHECK(currentStatus.engine, BIT_ENGINE_ACC) )
    if ( BIT_CHECK(currentStatus.engine, BIT_ENGINE_ACC) && (configPage15.swAE == 1) )         //[PJSC v1.10]
    {
      //AE Adds % of req_fuel
...
//[PJSC v1.10]  if( (configPage10.stagingEnabled == true) && (configPage2.nCylinders <= INJ_CHANNELS || configPage2.injType == INJ_TYPE_TBODY) && (currentStatus.PW1 > inj_opentime_uS) ) //Final check is to ensure that DFCO isn't active, which would cause an overflow below (See #267)
//[PJSC v1.10]  {
//[PJSC v1.10]    //Scale the 'full' pulsewidth by each of the injector capacities
//[PJSC v1.10]    currentStatus.PW1 -= inj_opentime_uS; //Subtract the opening time from PW1 as it needs to be multiplied out again by the pri/sec req_fuel values below. It is added on again after that calculation. 
//[PJSC v1.10]    uint32_t tempPW1 = div100((uint32_t)currentStatus.PW1 * staged_req_fuel_mult_pri);
//[PJSC v1.10]
//[PJSC v1.10]    if(configPage10.stagingMode == STAGING_MODE_TABLE)
//[PJSC v1.10]    {
//[PJSC v1.10]      uint32_t tempPW3 = div100((uint32_t)currentStatus.PW1 * staged_req_fuel_mult_sec); //This is ONLY needed in in table mode. Auto mode only calculates the difference.
//[PJSC v1.10]
//[PJSC v1.10]      uint8_t stagingSplit = get3DTableValue(&stagingTable, currentStatus.fuelLoad, currentStatus.RPM);
//[PJSC v1.10]      currentStatus.PW1 = div100((100U - stagingSplit) * tempPW1);
//[PJSC v1.10]      currentStatus.PW1 += inj_opentime_uS; 
//[PJSC v1.10]
//[PJSC v1.10]      //PW2 is used temporarily to hold the secondary injector pulsewidth. It will be assigned to the correct channel below
//[PJSC v1.10]      if(stagingSplit > 0) 
//[PJSC v1.10]      { 
//[PJSC v1.10]        BIT_SET(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE); //Set the staging active flag
//[PJSC v1.10]        currentStatus.PW2 = div100(stagingSplit * tempPW3); 
//[PJSC v1.10]        currentStatus.PW2 += inj_opentime_uS;
//[PJSC v1.10]      }
//[PJSC v1.10]      else
//[PJSC v1.10]      {
//[PJSC v1.10]        BIT_CLEAR(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE); //Clear the staging active flag
//[PJSC v1.10]        currentStatus.PW2 = 0; 
//[PJSC v1.10]      }
//[PJSC v1.10]    }
//[PJSC v1.10]    else if(configPage10.stagingMode == STAGING_MODE_AUTO)
//[PJSC v1.10]    {
//[PJSC v1.10]      currentStatus.PW1 = tempPW1;
//[PJSC v1.10]      //If automatic mode, the primary injectors are used all the way up to their limit (Configured by the pulsewidth limit setting)
//[PJSC v1.10]      //If they exceed their limit, the extra duty is passed to the secondaries
//[PJSC v1.10]      if(tempPW1 > pwLimit)
//[PJSC v1.10]      {
//[PJSC v1.10]        BIT_SET(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE); //Set the staging active flag
//[PJSC v1.10]        uint32_t extraPW = tempPW1 - pwLimit + inj_opentime_uS; //The open time must be added here AND below because tempPW1 does not include an open time. The addition of it here takes into account the fact that pwLlimit does not contain an allowance for an open time. 
//[PJSC v1.10]        currentStatus.PW1 = pwLimit;
//[PJSC v1.10]        currentStatus.PW2 = udiv_32_16(extraPW * staged_req_fuel_mult_sec, staged_req_fuel_mult_pri); //Convert the 'left over' fuel amount from primary injector scaling to secondary
//[PJSC v1.10]        currentStatus.PW2 += inj_opentime_uS;
//[PJSC v1.10]      }
//[PJSC v1.10]      else 
//[PJSC v1.10]      {
//[PJSC v1.10]        //If tempPW1 < pwLImit it means that the entire fuel load can be handled by the primaries and staging is inactive. 
//[PJSC v1.10]        currentStatus.PW1 += inj_opentime_uS; //Add the open time back in
//[PJSC v1.10]        BIT_CLEAR(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE); //Clear the staging active flag 
//[PJSC v1.10]        currentStatus.PW2 = 0; //Secondary PW is simply set to 0 as it is not required
//[PJSC v1.10]      } 
//[PJSC v1.10]    }
//[PJSC v1.10]
//[PJSC v1.10]    //Allocate the primary and secondary pulse widths based on the fuel configuration
//[PJSC v1.10]    switch (configPage2.nCylinders) 
//[PJSC v1.10]    {
//[PJSC v1.10]      case 1:
//[PJSC v1.10]        //Nothing required for 1 cylinder, channels are correct already
//[PJSC v1.10]        break;
//[PJSC v1.10]      case 2:
//[PJSC v1.10]        //Primary pulsewidth on channels 1 and 2, secondary on channels 3 and 4
//[PJSC v1.10]        currentStatus.PW3 = currentStatus.PW2;
//[PJSC v1.10]        currentStatus.PW4 = currentStatus.PW2;
//[PJSC v1.10]        currentStatus.PW2 = currentStatus.PW1;
//[PJSC v1.10]        break;
//[PJSC v1.10]      case 3:
//[PJSC v1.10]        //6 channels required for 'normal' 3 cylinder staging support
//[PJSC v1.10]        #if INJ_CHANNELS >= 6
//[PJSC v1.10]          //Primary pulsewidth on channels 1, 2 and 3, secondary on channels 4, 5 and 6
//[PJSC v1.10]          currentStatus.PW4 = currentStatus.PW2;
//[PJSC v1.10]          currentStatus.PW5 = currentStatus.PW2;
//[PJSC v1.10]          currentStatus.PW6 = currentStatus.PW2;
//[PJSC v1.10]        #else
//[PJSC v1.10]          //If there are not enough channels, then primary pulsewidth is on channels 1, 2 and 3, secondary on channel 4
//[PJSC v1.10]          currentStatus.PW4 = currentStatus.PW2;
//[PJSC v1.10]        #endif
//[PJSC v1.10]        currentStatus.PW2 = currentStatus.PW1;
//[PJSC v1.10]        currentStatus.PW3 = currentStatus.PW1;
//[PJSC v1.10]        break;
//[PJSC v1.10]      case 4:
//[PJSC v1.10]        if( (configPage2.injLayout == INJ_SEQUENTIAL) || (configPage2.injLayout == INJ_SEMISEQUENTIAL) )
//[PJSC v1.10]        {
//[PJSC v1.10]          //Staging with 4 cylinders semi/sequential requires 8 total channels
//[PJSC v1.10]          #if INJ_CHANNELS >= 8
//[PJSC v1.10]            currentStatus.PW5 = currentStatus.PW2;
//[PJSC v1.10]            currentStatus.PW6 = currentStatus.PW2;
//[PJSC v1.10]            currentStatus.PW7 = currentStatus.PW2;
//[PJSC v1.10]            currentStatus.PW8 = currentStatus.PW2;
//[PJSC v1.10]
//[PJSC v1.10]            currentStatus.PW2 = currentStatus.PW1;
//[PJSC v1.10]            currentStatus.PW3 = currentStatus.PW1;
//[PJSC v1.10]            currentStatus.PW4 = currentStatus.PW1;
//[PJSC v1.10]          #else
//[PJSC v1.10]            //This is an invalid config as there are not enough outputs to support sequential + staging
//[PJSC v1.10]            //Put the staging output to the non-existent channel 5
//[PJSC v1.10]            currentStatus.PW5 = currentStatus.PW2;
//[PJSC v1.10]          #endif
//[PJSC v1.10]        }
//[PJSC v1.10]        else
...
//[PJSC v1.10]        
//[PJSC v1.10]      case 5:
//[PJSC v1.10]        //No easily supportable 5 cylinder staging option unless there are at least 5 channels
//[PJSC v1.10]        #if INJ_CHANNELS >= 5
//[PJSC v1.10]          if (configPage2.injLayout != INJ_SEQUENTIAL)
//[PJSC v1.10]          {
//[PJSC v1.10]            currentStatus.PW5 = currentStatus.PW2;
//[PJSC v1.10]          }
//[PJSC v1.10]          #if INJ_CHANNELS >= 6
...
//[PJSC v1.10]      case 6:
//[PJSC v1.10]        #if INJ_CHANNELS >= 6
//[PJSC v1.10]          //8 cylinder staging only if not sequential
//[PJSC v1.10]          if (configPage2.injLayout != INJ_SEQUENTIAL)
//[PJSC v1.10]          {
//[PJSC v1.10]            currentStatus.PW4 = currentStatus.PW2;
//[PJSC v1.10]            currentStatus.PW5 = currentStatus.PW2;
...
//[PJSC v1.10]          else
//[PJSC v1.10]            {
//[PJSC v1.10]              //If there are 8 channels, then the 6 cylinder sequential option is available by using channels 7 + 8 for staging
//[PJSC v1.10]              currentStatus.PW7 = currentStatus.PW2;
//[PJSC v1.10]              currentStatus.PW8 = currentStatus.PW2;
//[PJSC v1.10]
//[PJSC v1.10]              currentStatus.PW4 = currentStatus.PW1;
//[PJSC v1.10]              currentStatus.PW5 = currentStatus.PW1;
//[PJSC v1.10]              currentStatus.PW6 = currentStatus.PW1;
//[PJSC v1.10]            }
//[PJSC v1.10]          #endif
//[PJSC v1.10]        #endif
//[PJSC v1.10]        currentStatus.PW2 = currentStatus.PW1;
...
//[PJSC v1.10]
//[PJSC v1.10]      default:
//[PJSC v1.10]        //Assume 4 cylinder non-seq for default
//[PJSC v1.10]        currentStatus.PW3 = currentStatus.PW2;
//[PJSC v1.10]        currentStatus.PW4 = currentStatus.PW2;
//[PJSC v1.10]        currentStatus.PW2 = currentStatus.PW1;
//[PJSC v1.10]        break;
...
//[PJSC v1.10]  else 
//[PJSC v1.10]  { 
//[PJSC v1.10]    if(maxInjOutputs >= 2) { currentStatus.PW2 = currentStatus.PW1; }
//[PJSC v1.10]    else { currentStatus.PW2 = 0; }
//[PJSC v1.10]    if(maxInjOutputs >= 3) { currentStatus.PW3 = currentStatus.PW1; }
//[PJSC v1.10]    else { currentStatus.PW3 = 0; }
//[PJSC v1.10]    if(maxInjOutputs >= 4) { currentStatus.PW4 = currentStatus.PW1; }
//[PJSC v1.10]    else { currentStatus.PW4 = 0; }
//[PJSC v1.10]    if(maxInjOutputs >= 5) { currentStatus.PW5 = currentStatus.PW1; }
//[PJSC v1.10]    else { currentStatus.PW5 = 0; }
//[PJSC v1.10]    if(maxInjOutputs >= 6) { currentStatus.PW6 = currentStatus.PW1; }
//[PJSC v1.10]    else { currentStatus.PW6 = 0; }
//[PJSC v1.10]    if(maxInjOutputs >= 7) { currentStatus.PW7 = currentStatus.PW1; }
//[PJSC v1.10]    else { currentStatus.PW7 = 0; }
//[PJSC v1.10]    if(maxInjOutputs >= 8) { currentStatus.PW8 = currentStatus.PW1; }
//[PJSC v1.10]    else { currentStatus.PW8 = 0; }
//[PJSC v1.10]
//[PJSC v1.10]    BIT_CLEAR(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE); //Clear the staging active flag
//[PJSC v1.10]    
//[PJSC v1.10]  } 
      //************************************************* [PJSC v1.10] *********************************************************************
      if( (configPage10.stagingEnabled == true) && (configPage2.nCylinders <= INJ_CHANNELS || configPage2.injType == INJ_TYPE_TBODY) && (currentStatus.PW1 > inj_opentime_uS) )
      {
        calculateStagingPW(currentStatus.PW2, inj2_opentime_uS, inj4_opentime_uS, pwLimit);
        primaryPW_2 = primaryPW;
        secondaryPW_2 = secondaryPW;

        calculateStagingPW(currentStatus.PW1, inj_opentime_uS, inj3_opentime_uS, pwLimit);
        ...
        switch (configPage2.nCylinders) 
        {
          ...
          case 2:
            ...
            currentStatus.PW4 = secondaryPW_2;
            currentStatus.PW2 = primaryPW_2;
            break;
          case 3:
          ...
          case 4:
            if( (configPage2.injLayout == INJ_SEQUENTIAL) || (configPage2.injLayout == INJ_SEMISEQUENTIAL) )
            {
              ...
              #if INJ_CHANNELS >= 8
                currentStatus.PW5 = secondaryPW;
                currentStatus.PW6 = secondaryPW_2;
                currentStatus.PW7 = secondaryPW;
                currentStatus.PW8 = secondaryPW_2;

                currentStatus.PW2 = primaryPW_2;
                currentStatus.PW3 = primaryPW;
                currentStatus.PW4 = primaryPW_2;
              #else
      ...
      { 
        ...
        else
        {
          ...
          {
            if( currentStatus.PW2 != 0 ){ currentStatus.PW2 = currentStatus.PW2 - inj_opentime_uS + inj2_opentime_uS; }
            if( currentStatus.PW3 != 0 ){ currentStatus.PW3 = currentStatus.PW3 - inj_opentime_uS + inj3_opentime_uS; }
            if( currentStatus.PW4 != 0 ){ currentStatus.PW4 = currentStatus.PW4 - inj_opentime_uS + inj4_opentime_uS; }
          }
        }
      }
...
//******************************************** [PJSC v1.10] ******************************************************
void calculateStagingPW(uint16_t PW, int injOpenPrim, int injOpenSecond, unsigned long pwLimit)
{
  uint32_t tempPW = (unsigned long)PW;
  ...
  if(configPage10.stagingMode == STAGING_MODE_TABLE)
  {
    ...
    tempPW -= injOpenPrim;
    primaryPW = ((100 - stagingSplit) * tempPW) / 100;
    primaryPW += injOpenPrim; 

    //Secondary PW is used temporarily to hold the secondary injector pulsewidth. It will be assigned to the correct channel below
    if(stagingSplit > 0)
    {
      BIT_SET(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE); //Set the staging active flag
      secondaryPW = (stagingSplit * tempPW * staged_req_fuel_mult_sec) / staged_req_fuel_mult_pri / 100;
      secondaryPW += injOpenSecond;
    }
    else
    {
  ...
  else if(configPage10.stagingMode == STAGING_MODE_AUTO)
  {
    ...
    {
      BIT_SET(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE); //Set the staging active flag
      uint32_t extraPW = tempPW - pwLimit;
      primaryPW = pwLimit;
    ...
    {
      //If tempPW < pwLImit it means that the entire fuel load can be handled by the primaries and staging is inactive. 
