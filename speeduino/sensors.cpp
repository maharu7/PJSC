...
void initialiseADC(void)
{
  ...
  if(configPage15.ADCFILTER_EGT > 240U) { configPage15.ADCFILTER_EGT  = ADCFILTER_EGT_DEFAULT;   writeConfig(boostvvtPage2); }      //[PJSC v1.10]
  if(configPage15.ADCFILTER_PV  > 240U) { configPage15.ADCFILTER_PV   = ADCFILTER_PV_DEFAULT;    writeConfig(boostvvtPage2); }      //[PJSC v1.10]
...
{
  current.MAP10 = mapADCToMAP(readings.mapADC, (page2.mapMin * 10), (page2.mapMax * 10));           //[PJSC v1.10]
...
//[PJSC v1.10]  static constexpr uint16_t BARO_MAX = 108U;
  static constexpr uint16_t BARO_MAX = 140U;    //[PJSC v1.10]
...
static inline void setBaroFromSensorReading(uint16_t sensorReading) 
{
  ...
  currentStatus.baro10 = fastMap10Bit(currentStatus.baroADC, (configPage2.baroMin * 10), (configPage2.baroMax * 10));     //[PJSC v1.10]
}
...
//[PJSC] For PV control
void readPVPosition(bool useFilter)
{
  //uint16_t tempReading = fastMap10Bit(readAnalogSensor(pinExValve), configPage15.PVPosMin, configPage15.PVPosMax); //Get the current raw PVP ADC value
  uint16_t tempReading = fastMap10Bit(readAnalogSensor(pinExValve), 0U, 511U); //Get the current raw PVP ADC value

  //The use of the filter can be overridden if required. This is used on startup to disable priming pulse if flood clear is wanted
  if(useFilter == true) { currentStatus.PVPositionADC = (uint16_t)LOW_PASS_FILTER((uint16_t)tempReading, configPage15.ADCFILTER_PV, (uint16_t)currentStatus.PVPositionADC); }
  else { currentStatus.PVPositionADC = tempReading; }
  uint16_t tempADC = currentStatus.PVPositionADC;

  if(configPage15.PVPosMax > configPage15.PVPosMin)
  {
    //Check that the ADC values fall within the min and max ranges (Should always be the case, but noise can cause these to fluctuate outside the defined range).
    tempADC = clamp(tempADC, configPage15.PVPosMin, configPage15.PVPosMax);
    currentStatus.PVPosition = map(tempADC, configPage15.PVPosMin, configPage15.PVPosMax, 0, 200);
  }
  else
  {
    tempADC = UINT16_MAX - tempADC; //Reverse the ADC values
    uint16_t tempPVPMax = UINT16_MAX - configPage15.PVPosMax;
    uint16_t tempPVPMin = UINT16_MAX - configPage15.PVPosMin;

    //All checks below are reversed from the standard case above
    tempADC = clamp(tempADC, tempPVPMin, tempPVPMax);
    currentStatus.PVPosition = map(tempADC, tempPVPMin, tempPVPMax, 0, 200);
  }
}

//[PJSC v1.10] For Exhaust Gas Temperature sennsor
void readEGT(void)
{
  uint16_t tempADC = LOW_PASS_FILTER(readAnalogSensor(pinEGT), configPage15.ADCFILTER_EGT, currentStatus.EGTADC);
  currentStatus.EGTADC = tempADC;
}
